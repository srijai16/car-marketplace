import Data from '@/Shared/Data'
import React from 'react'
import { Link } from 'react-router-dom'

export default function Category() {
  return (
    <div className='mt-40'>
      <h2 className='font-bold text-3xl text-center mb-6'>Browse By Type</h2>

      <div className='grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-9 gap-6 px-4 sm:px-20'>
        {Data.Category.map((Category, index) => (
          <Link key={index} to={'search/' + Category.name}>
            <div className='border rounded-xl p-3 items-center flex flex-col hover:shadow-md cursor-pointer h-full'>
              
              <img src={Category.icon} width={40} height={40} />

              <h2 className='mt-2 text-center text-xs sm:text-sm break-words'>
                {Category.name}
              </h2>

            </div>
          </Link>
        ))}
      </div>
    </div>
  )
}